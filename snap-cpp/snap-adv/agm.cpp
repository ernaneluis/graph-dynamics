...
/// dump membership for each user (Size: Nodes * Coms) into a text file
void TAGMUtil::DumpF(const TStr OutFNm, const TVec<TIntFltH>& CmtyVV) {
  FILE* file1 = fopen(OutFNm.CStr(),"wt");

  for (int i = 0; i < CmtyVV.Len(); i++) {
    // CmtyVV[i] is type THash<TInt, TFlt, TDefaultHashFunc<TInt> >
    if (CmtyVV[i].Len() == 0){
      fprintf(file1,"%g", (double) 0.0001);
    }

    for (int j = 0; j < CmtyVV[i].Len(); j++) {
      fprintf(file1,"%g\t", (double) CmtyVV[i][j]);
    }
    fprintf(file1,"\n");
  }
  fclose(file1);
